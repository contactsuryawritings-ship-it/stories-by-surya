// Adapted from the React Bits JS/Tailwind source supplied with this project.
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // A shared slot keeps looping/snap positions stable; each plane has its own aspect.
    const slotWidth = Math.min(this.viewport.width * 0.78, this.viewport.height * 0.9);
    const width = Math.min(slotWidth, this.viewport.height * 0.6 * this.aspect);
    this.plane.scale.x = width;
    this.plane.scale.y = width / this.aspect;
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    if (this.title) {
      const textHeight = this.viewport.height * 0.035;
      this.title.mesh.scale.set(
        textHeight / this.plane.scale.x,
        textHeight / this.plane.scale.y,
        1,
      );
      this.title.mesh.position.y = -0.5 - textHeight / this.plane.scale.y;
    }
    this.padding = this.viewport.height * 0.06;
...
class App {
  constructor(
    ...
    this.visibility = observeVisibility(container);
  ...
  navigate(direction) {
    ...
    this.scroll.target = (Math.round(this.scroll.target / width) + direction) * width;
  }
  onCheck() {
  ...
  update() {
    ...
    const index =
      ((Math.round(this.scroll.current / this.medias[0].width) % this.itemCount) + this.itemCount) %
      this.itemCount;
  ...
  addEventListeners() {
    ...
    this.container.addEventListener("pointercancel", this.onTouchUp);
  ...
  destroy() {
    ...
    this.container.removeEventListener("pointercancel", this.onTouchUp);
    this.container.removeEventListener("keydown", this.onKeyDown);
    this.medias.forEach((media) => {
      media.imageElement.onload = media.imageElement.onerror = null;
    });
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onError]);
  return (
    <div className="relative h-full w-full">
      <div
        style={{ touchAction: "pan-y", height: "calc(100% - 4rem)" }}
      ...
      <div className="absolute inset-x-4 bottom-4 flex items-center justify-between text-white">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() => appRef.current?.navigate(-1)}
          >
            ←
          </button>
          <button
      ...
      <p className="sr-only">{items[index]?.alt || items[index]?.text}</p>
    </div>
  );
}
