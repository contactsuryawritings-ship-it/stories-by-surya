...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const ratio =
        imageSizesRef.current[item.image] ||
        (item.width && item.height ? item.width / item.height : 1);
      const width = Math.min(
        rootRef.current.clientWidth * 0.72,
        640,
        Math.min(rootRef.current.clientHeight * 0.8, 440) * ratio,
      );
      ...
      el.style.transform = `translate(-50%, -50%) translateX(${(tx * sc).toFixed(2)}px) translateZ(${tz.toFixed(2)}px) rotateY(${ry.toFixed(3)}deg)`;
          ...
          >
            <img
              ...
              onLoad={(event) => {
                ...
                imageSizesRef.current[item.image] = image.naturalWidth / image.naturalHeight;
                layout(posRef.current);
              }}
              alt={item.alt || ""}
              draggable={false}
              loading={Math.abs(i - active) <= visibleCards ? "eager" : "lazy"}
              decoding="async"
            />
            <span
